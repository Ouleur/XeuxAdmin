...
def send_email_with_file(to,cc, subject, template,document, **kwargs):
    ...
    msg = Message(app.config['ONE_MAIL_SUBJECT_PREFIX'] + subject,sender=app.config['ONE_MAIL_SENDER'], recipients=[to])
    msg.body = render_template('email/'+template + '.txt', **kwargs)
    msg.html = render_template('email/'+template + '.html', **kwargs)
    msg.cc = []
    working_dir = '/var/www/one4driver/app/static/uploads/'
...
def send_email(to,cc, subject, template,document, **kwargs):
    app = current_app._get_current_object()
    msg = Message(app.config['ONE_MAIL_SUBJECT_PREFIX'] + subject,sender=app.config['ONE_MAIL_SENDER'], recipients=[to])
    msg.body = render_template('email/'+template+ '.txt', **kwargs)
    msg.html = render_template('email/'+template+ '.html', **kwargs)
